...
const createHotel = async (req, res, next) => {
  try {
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Hotel added successfully." });
    } catch (error) {
      console.log(error);
...
const deleteHotel = async (req, res, next) => {
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM unavailableDates WHERE roomNumber_id IN (SELECT rn.id FROM roomNumbers rn JOIN rooms r ON rn.room_id = r.id WHERE r.hotel_id = ?)",
      [hotelId]
    );

    // Delete from 'roomNumbers' table
    await pool.query(
      "DELETE FROM roomNumbers WHERE room_id IN (SELECT id FROM rooms WHERE hotel_id = ?)",
      [hotelId]
    );
...
const getHotel = async (req, res, next) => {
  const hotelId = req.params.id;
  try {
    const [hotel] = await pool.query("SELECT * FROM hotels WHERE id = ?", [
      hotelId,
    ]);
...
const countByType = async (req, res, next) => {
  try {
    const [hotelCountRows, hotelCountFields] = await pool.query(
      'SELECT COUNT(*) AS count FROM hotels WHERE type = "hotel"'
    );
    const [apartmentCountRows, apartmentCountFields] = await pool.query(
      'SELECT COUNT(*) AS count FROM hotels WHERE type = "apartment"'
    );
    const [resortCountRows, resortCountFields] = await pool.query(
      'SELECT COUNT(*) AS count FROM hotels WHERE type = "resort"'
    );
    const [villaCountRows, villaCountFields] = await pool.query(
      'SELECT COUNT(*) AS count FROM hotels WHERE type = "villa"'
    );
    const [cabinCountRows, cabinCountFields] = await pool.query(
      'SELECT COUNT(*) AS count FROM hotels WHERE type = "cabin"'
    );
...
const getHotelRooms = async (req, res, next) => {
  try {
    ...
    const roomsWithRoomNumbers = await Promise.all(
      list.map(async (room) => {
        const [roomNumbersData] = await pool.query(
          "SELECT * FROM roomNumbers WHERE room_id = (?)",
          [room.id]
        );
        ...
        const roomNumbersWithUnavailableDates = await Promise.all(
          roomNumbersData.map(async (roomNumber) => {
            const [unavailableDatesData] = await pool.query(
              "SELECT * FROM unavailableDates WHERE roomNumber_id = (?)",
              [roomNumber.id]
            );
...
const updateRoomAvailability = async (req, res, next) => {
  ...
  if (!Array.isArray(timestamps)) {
    return res
      .status(400)
      .json({ error: "Dates should be an array of timestamps" });
  }
  ...
  try {
    ...
    await Promise.all(
      timestamps.map(async (timestamp) => {
        ...
        await pool.query(
          "INSERT INTO unavailableDates (roomNumber_id, date, user_id) VALUES (?, ?, ?)",
          [roomNumberId, date, userId]
        );
      })
    );
    ...
    await pool.query(
      "UPDATE roomNumbers SET updated_at = CURRENT_TIMESTAMP WHERE id = ?",
      [roomNumberId]
    );
...
const updateRoom = async (req, res, next) => {
  ...
  try {
    ...
    const [selectedRoom] = await pool.query(
...
const deleteRoom = async (req, res, next) => {
  ...
  try {
    // Delete from 'unavailableDates' table
    await pool.query(
      "DELETE FROM unavailableDates WHERE roomNumber_id IN (SELECT id FROM roomNumbers WHERE room_id = ?)",
      [roomId]
    );
...
const getRoom = async (req, res, next) => {
  ...
  try {
    const [selectedRoom] = await pool.query(
      "SELECT rooms.id ,rooms.title, rooms.price, rooms.maxPeople, rooms.description FROM rooms JOIN hotels ON rooms.hotel_id = hotels.id WHERE rooms.hotel_id = (?)",
      [roomId]
    );
...
`,
    ...
    if (rows.length === 0) {
      return res
        .status(200)
        .json({ message: "No reservations found for the user." });
    }
...
const delReserved = async (req, res) => {
  try {
    ...
    // Check if user exists
    const [rows, fields] = await pool.execute(
      "DELETE FROM unavailableDates WHERE id = ?",
      [unavailId]
    );
